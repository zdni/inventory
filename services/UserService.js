...
class UserService {
  async generateData(req) {
    try {
      ...
      const isEmailExist = await emailExist(username)
      if(isEmailExist) { return { status: false, code: 409, message: "USERNAME_EXIST" } }
  ...
  async processResetPassword(req) {
    try {
      ...
      const checkUserId = await checkValidationObjectId(id, User, "USER")
